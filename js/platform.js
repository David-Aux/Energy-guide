    ...
    async function ensureSignedInEndUser(forceRefresh = false) {
      try {
        ...
        if (!forceRefresh && currentUser && currentUser.role === 'user' && String(currentUser.id) === String(authUser.id)) {
    ...
    function updateUserAccountUI() {
      ...
      if (isSignedInEndUser()) {
        const name = currentUser.full_name || currentUser.company_name || currentUser.email || 'Your account';
    ...
    function switchUserAuthMode(mode) {
      ...
      const fullNameGroup = document.getElementById('userAuthFullName')?.closest('.form-group');
    ...
    async function handleUserAuthSubmit() {
      ...
      currentUser = { ...(authUser || {}), ...(profile || {}), role: 'user', full_name: profile?.full_name || fullName || authUser?.user_metadata?.full_name || '' };
      currentRole = 'user';
      closeUserAuthModal();
