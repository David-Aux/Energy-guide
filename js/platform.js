    ...
    function forceResetScroll(screenId = null) {
      const topBar = document.getElementById('topBar');
      const screen = screenId ? document.getElementById(screenId) : document.querySelector('.screen.active');
      ...
      const resetNow = () => {
        ...
        try {
          if (topBar && typeof topBar.scrollIntoView === 'function') {
            topBar.scrollIntoView({ block: 'start', inline: 'nearest', behavior: 'auto' });
          }
        } catch(e) {}
      };
      ...
      requestAnimationFrame(() => {
        resetNow();
        setTimeout(resetNow, 0);
        setTimeout(resetNow, 60);
        setTimeout(resetNow, 180);
        setTimeout(resetNow, 420);
      });
    }
