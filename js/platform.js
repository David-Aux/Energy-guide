      ...
      try {
        ...
        container.innerHTML = calcs.map(calc => `
          <div class="saved-calc-item">
            <div style="font-weight: 600;">${Math.ceil((calc.requiredInverter || 0) / 1000)}kW System</div>
    ...
    function downloadSystemPDF() {
      if (!userCalculationResult) {
        showToast('No system to export', 'error');
        return;
      }
      ...
      try {
        ...
        doc.setFontSize(18);
        doc.text('Energy Guide - Solar System Report', 14, 18);

        doc.setFontSize(12);
        doc.text(`Inverter: ${r.invKva || '-'}kVA`, 14, 34);
        doc.text(`Panels: ${r.numPanels || 0} x 500W`, 14, 44);
        doc.text(`Battery: ${r.lithiumPackKwh || 0} kWh Lithium`, 14, 54);
        doc.text(`Daily Energy: ${Number(r.dailyKwh || 0).toFixed(2)} kWh`, 14, 64);
        doc.text(`Estimated Cost: ₦${Number(r.totalCost || 0).toLocaleString()}`, 14, 74);

        doc.setFontSize(10);
        doc.text('Generated via Energy Guide', 14, 92);
    ...
    function shareSystemWhatsApp() {
      if (!userCalculationResult) {
        showToast('No system to share', 'error');
        return;
      }
      const r = userCalculationResult;
      const message = [
        'Energy Guide Solar Recommendation ☀️',
        '',
        `Inverter: ${r.invKva || '-'}kVA`,
        `Panels: ${r.numPanels || 0} x 500W`,
        `Battery: ${r.lithiumPackKwh || 0} kWh Lithium`,
        `Daily Load: ${Number(r.dailyKwh || 0).toFixed(2)} kWh`,
        '',
        `Estimated Cost: ₦${Number(r.totalCost || 0).toLocaleString()}`,
        '',
    ...
    function updateSavedSystemActionUI() {
      ...
      const pdfBtn = document.getElementById('savedSystemPdfBtn');
      const waBtn = document.getElementById('savedSystemWhatsappBtn');
      const delBtn = document.getElementById('savedSystemDeleteBtn');
      if (banner) banner.style.display = hasSaved ? 'block' : 'none';
      if (pdfBtn) pdfBtn.style.display = hasSaved ? 'block' : 'none';
      if (waBtn) waBtn.style.display = hasSaved ? 'block' : 'none';
      if (delBtn) delBtn.style.display = hasSaved ? 'block' : 'none';
    }
    ...
    function buildSavedSystemShareMessage(calc) {
      ...
      const data = { ...(calc || {}), ...payload };
      const inv = data.invKva || data.inverter_kva || '-';
      const panels = data.numPanels || data.panel_count || 0;
      const battery = data.lithiumPackKwh || data.battery_kwh || 0;
      const daily = Number(data.dailyKwh || data.daily_kwh || 0).toFixed(2);
      const cost = Number(data.totalCost || data.total_cost || 0).toLocaleString();

      return [
        'Energy Guide Solar Recommendation ☀️',
        '',
        `Inverter: ${inv}kVA`,
        `Panels: ${panels} x 500W`,
        `Battery: ${battery} kWh Lithium`,
        `Daily Load: ${daily} kWh`,
        '',
        `Estimated Cost: ₦${cost}`,
        '',
        'Generated via Energy Guide'
    ...
    function downloadSavedSystemPDF(cloudId) {
      getSavedCalculationsSafe().then(calcs => {
        ...
        doc.setFontSize(18);
        doc.text('Energy Guide - Saved Solar System', 14, 18);

        doc.setFontSize(12);
        doc.text(`Inverter: ${data.invKva || data.inverter_kva || '-'}kVA`, 14, 34);
        doc.text(`Panels: ${data.numPanels || data.panel_count || 0} x 500W`, 14, 44);
        doc.text(`Battery: ${data.lithiumPackKwh || data.battery_kwh || 0} kWh Lithium`, 14, 54);
        doc.text(`Daily Energy: ${Number(data.dailyKwh || data.daily_kwh || 0).toFixed(2)} kWh`, 14, 64);
        doc.text(`Estimated Cost: ₦${Number(data.totalCost || data.total_cost || 0).toLocaleString()}`, 14, 74);
        doc.setFontSize(10);
        doc.text(`Saved: ${calc.timestamp ? new Date(calc.timestamp).toLocaleString() : ''}`, 14, 92);

        doc.save('energy-guide-saved-system.pdf');
        showToast('PDF downloaded', 'success');
      }).catch(err => {
...
async function supabaseSubmitMarketplaceLead(lead) {
  try {
    ...
    */
    ...
    document.addEventListener('DOMContentLoaded', () => { setTimeout(() => { if (typeof updateUserAccountUI === 'function') updateUserAccountUI(); }, 50); });
