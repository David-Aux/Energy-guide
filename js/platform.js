    ...
    async function saveCurrentSystemForUser(skipPrompt = false) {
      ...
      try {
        ...
        showLoading(false);
        updateUserAccountUI();
        const saveMsg = document.getElementById('userSaveSuccessSubtitle');
        if (saveMsg) saveMsg.textContent = 'Your calculation has been saved to your account.';
        showScreen('user-save-success');
    ...
    function loadUserSavedCalculation(cloudId) {
      ...
      getSavedCalculationsSafe().then(calcs => {
        ...
        showLoading(false);
        showScreen('user-calculator');
        showToast('Saved system opened.', 'success');
      }).catch(err => {
        showLoading(false);
