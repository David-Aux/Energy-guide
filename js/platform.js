    ...
    function handleUserCalcResult(result, apps = []) {
      ...
      if (actionEl) {
        cleanupUserCalculatorActionArea();
    ...
    function cleanupUserCalculatorActionArea() {
      const actionEl = document.getElementById('userPostCalcActions');
      if (!actionEl) return;
      const savedOnlySelectors = [
        '[data-saved-only="true"]',
        '[onclick*="downloadSystemPDF"]',
        '[onclick*="shareSystemWhatsApp"]',
        '[onclick*="deleteCurrentOpenedSavedSystem"]',
        '[onclick*="deleteSavedSystem"]'
      ];
      savedOnlySelectors.forEach(sel => {
        actionEl.querySelectorAll(sel).forEach(el => el.remove());
      });
    }
