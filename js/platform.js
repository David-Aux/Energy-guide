    ...
    async function renderJobLeads() {
      ...
      const leads = (allLeads || []).filter(lead => {
        const isUnclaimed = !lead.claimed_by || lead.claim_status === 'open' || !lead.claim_status;
      ...
      container.innerHTML = leads.map(lead => {
        ...
        if (isClosed) {
          ...
          buttons = `<button class="btn btn-success small-btn" onclick="claimLead('${lead.id}')">🔒 Claim Lead</button>`;
        } else if (isMine) {
          buttons = `
