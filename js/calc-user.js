  ...
  function l4u__renderResults(s){
    ...
    set('l4u-s-inverter-voltage',s.V+'V DC System');
    ...
    set('l4u-s-battery-config',b0.wiring||'-');
  ...
  function l4u_calculate(){if(!l4u_appliances.length)return;l4u__egS=l4u__runEngine(l4u_appliances);l4u__applyPanelMode(l4u__egS,'A');l4u__egS=l4u__buildBOM(l4u__egS,0);l4u__renderResults(l4u__egS);
    try {
  ...
  function l4u_resetSession(){
    l4u_appliances = [];
    l4u__egS = null;

    var list = document.getElementById('l4u-appliance-list');
    if (list) {
      list.innerHTML = '<div class="empty-state"><div class="icon">🏠</div><p>No appliances added yet.<br/>Use the form or quick-add chips above.</p></div>';
    }

    var count = document.getElementById('l4u-count');
    if (count) count.textContent = '0';

    var btn = document.getElementById('l4u-calc-btn');
    if (btn) btn.disabled = true;

    var placeholder = document.getElementById('l4u-placeholder');
    if (placeholder) placeholder.style.display = 'flex';

    var results = document.getElementById('l4u-results');
    if (results) results.classList.remove('visible');
  }

  function l4u_loadSavedSystem(apps){
    l4u_resetSession();
    (apps || []).forEach(function(app){
      l4u_addItem({
        name: app.name,
        watts: Number(app.watts || 0),
        qty: Number(app.qty || app.quantity || 1),
        hours: Number(app.hours || app.hours_per_day || 0),
        inductive: !!app.inductive,
        dutyCycle: app.dutyCycle || null,
        fridgeMode: app.fridgeMode || null
      });
    });
