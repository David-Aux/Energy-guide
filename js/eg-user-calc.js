...
let egV_appliances = [];
  ...
  function egV_calculateSolarPanels(dailyWh, battery, rechargeStrategy = 'daily_full') {
    ...
    const numPanels = Math.ceil(egV_pvWatts / PANEL_500W.watts);

    const egV_dailyGenKwh = (numPanels * PANEL_500W.watts * psh * systemEff) / 1000;
  ...
  function egV_calculateEngine(apps, rechargeStrategy = 'daily_full') {
    ...
    const pvBreaker = egV_pickBreaker(pvDesignCurrent, PV_BREAKERS, 16);
    ...
    const battBreaker = egV_pickBreaker(battDesignCurrent, BATTERY_BREAKERS, 63);
    ...
    const acBreaker = egV_pickBreaker(acDesignCurrent, AC_BREAKERS, 16);
...
let egI_appliances = [];
  ...
  function egI_calculateSolarPanels(dailyWh, battery, rechargeStrategy = 'daily_full') {
    ...
    const numPanels = Math.ceil(egI_pvWatts / PANEL_500W.watts);

    const egI_dailyGenKwh = (numPanels * PANEL_500W.watts * psh * systemEff) / 1000;
  ...
  function egI_calculateEngine(apps, rechargeStrategy = 'daily_full') {
    ...
    const pvBreaker = egI_pickBreaker(pvDesignCurrent, PV_BREAKERS, 16);
    ...
    const battBreaker = egI_pickBreaker(battDesignCurrent, BATTERY_BREAKERS, 63);
    ...
    const acBreaker = egI_pickBreaker(acDesignCurrent, AC_BREAKERS, 16);
